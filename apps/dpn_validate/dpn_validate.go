...
// dpn_validate validates bags that we replicate from other nodes.
...
func main() {
	...
	consumer, err := apt_workers.CreateNsqConsumer(_context.Config, &_context.Config.DPN.DPNValidationWorker)
	if err != nil {
		_context.MessageLog.Fatalf(err.Error())
	}
	validator, err := workers.NewDPNValidator(_context)
	if err != nil {
		_context.MessageLog.Error(err.Error())
		fmt.Fprintf(os.Stderr, err.Error())
		os.Exit(1)
	}
	_context.MessageLog.Info("dpn_validate started")
...
Usage: dpn_validate -config=<path to APTrust config file>
